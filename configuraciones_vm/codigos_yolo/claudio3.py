...
# Zonas de pasajeros (para clasificación)
ZONA_PAX1 = [(862, 717), (1022, 569), (1030, 452), (995, 330), (657, 314), (681, 375), (471, 436), (286, 498), (141, 544), (2, 591), (2, 719), (306, 719), (550, 717)]
TRANSVERSALES1 = [(5, 2), (5, 1), (6, 0), (7, 12), (8, 11), (9, 11)]  # Pares de índices que conectan puntos
...
TIEMPO_OPERADOR = 90.0          # Segundos en escena para considerar operador (1.5 minutos)
...
PORCENTAJE_RECORRIDO_MIN = 0.7  # 70% de líneas transversales cruzadas para ser pasajero
...
class PersonTracker:
    ...
    is_passenger: bool = False  # Si cumple criterio de 70% en alguna zona PAX
    
    # Tracking de zonas PAX
    in_pax1: bool = False
    in_pax2: bool = False
    transversales_pax1_crossed: Set[int] = field(default_factory=set)  # Índices de transversales cruzadas
    ...
    def get_pax1_coverage(self) -> float:
        """Retorna porcentaje de cobertura en ZONA_PAX1 (0.0 a 1.0)."""
        total_lines = len(TRANSVERSALES1)
        if total_lines == 0:
            return 0.0
        return len(self.transversales_pax1_crossed) / total_lines
    
    def get_pax2_coverage(self) -> float:
        """Retorna porcentaje de cobertura en ZONA_PAX2 (0.0 a 1.0)."""
        total_lines = len(TRANSVERSALES2)
        if total_lines == 0:
            return 0.0
        return len(self.transversales_pax2_crossed) / total_lines
    
    def meets_passenger_criteria(self) -> bool:
        """Verifica si está en alguna zona PAX (criterio simplificado)."""
...
def check_transversal_crossings(prev_pos: Optional[Tuple[float, float]], 
                                curr_pos: Tuple[float, float],
                                zona: List[Tuple[int, int]],
                                transversales: List[Tuple[int, int]]) -> Set[int]:
    """Detecta qué líneas transversales fueron cruzadas.
    Retorna set de índices de transversales cruzadas."""
    if not prev_pos:
        return set()
...
class TelefericoCounterV2:
    ...
    def __init__(self, model_path: str, rtsp_url: str, output_folder: str,
        ...
        # Acumulador de total de personas embarcadas
    ...
    def _detect_operator(self, person: PersonTracker, current_time: float) -> bool:
        ...
        Criterio:
        1. Tiempo en escena >90s (1.5 minutos) = operador
        """
    ...
    def _update_persons(self, heads: List, current_time: float) -> List[int]:
        ...
        for head in heads:
            ...
            person.in_pax1 = in_pax1
            person.in_pax2 = in_pax2
            
            # Detectar cruces de líneas transversales
            if prev_pos:
                if in_pax1:
            ...
            # Verificar si cumple criterio de pasajero
            ...
            # Actualizar conjunto de personas en zona (solo pasajeros que no son operadores)
            if (in_zone or in_pax1 or in_pax2) and person.is_passenger and not person.is_operator:
    ...
    def _handle_cabin_event(self, cabin_track_id: int, current_time: float):
        ...
        if self.current_event:
            # Las personas que estaban en zona al inicio del ciclo son las que embarcaron
            embarked_ids = list(self.current_event.persons_before)
            ...
            # Acumular total
            self.total_embarked_count += len(embarked_ids)
    ...
    def _draw_annotations(self, frame: np.ndarray, heads: List, cabins: List) -> np.ndarray:
        ...
        # ZONA_PAX1 (azul semi-transparente)
        overlay = frame.copy()
        pts_pax1 = np.array(ZONA_PAX1, dtype=np.int32)
        cv2.fillPoly(overlay, [pts_pax1], (200, 100, 0))
        frame = cv2.addWeighted(overlay, 0.08, frame, 0.92, 0)
        cv2.polylines(frame, [pts_pax1], True, (255, 150, 0), 2)
        
        # Líneas transversales PAX1 (cyan)
        for p1_idx, p2_idx in TRANSVERSALES1:
            cv2.line(frame, ZONA_PAX1[p1_idx], ZONA_PAX1[p2_idx], (255, 255, 0), 1)
        
        # ZONA_PAX2 (magenta semi-transparente)
        overlay = frame.copy()
        pts_pax2 = np.array(ZONA_PAX2, dtype=np.int32)
        cv2.fillPoly(overlay, [pts_pax2], (100, 0, 200))
        frame = cv2.addWeighted(overlay, 0.08, frame, 0.92, 0)
        cv2.polylines(frame, [pts_pax2], True, (150, 0, 255), 2)
        
        # Líneas transversales PAX2 (amarillo)
        ...
        for head in heads:
            ...
            if tid and tid in self.persons:
                ...
                if person.is_operator:
                    ...
                    # Mostrar porcentaje de cobertura de la zona con mayor recorrido
                    pax1_cov = person.get_pax1_coverage()
                    pax2_cov = person.get_pax2_coverage()
                    max_cov = max(pax1_cov, pax2_cov)
                    color = (0, 255, 0)  # Verde = pasajero
                    label = f"PAX:{tid} ({tiempo:.1f}s {max_cov*100:.0f}%)"
                else:
                    color = (255, 165, 0)  # Naranja = sin clasificar
        ...
        y += 28
        pasajeros = len([p for p in self.persons.values() 
                        if p.is_passenger and not p.is_operator])
        cv2.putText(frame, f"Pasajeros (PAX): {pasajeros}", (20, y),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 1)
        
        y += 25
        sin_clasificar = len([p for p in self.persons.values() 
                             if not p.is_passenger and not p.is_operator])
        cv2.putText(frame, f"Sin clasificar: {sin_clasificar}", (20, y),
    ...
    def _save_minute_log(self, log_path: str, elapsed_time: float, frame_count: int):
        ...
        with open(log_path, 'a', newline='', encoding='utf-8') as f:
            ...
            operadores = len([p for p in self.persons.values() if p.is_operator])
            embarcados_total = self.total_embarked_count
    ...
    def run(self, duration_minutes: float):
        ...
        try:
            ...
            total_embarked = self.total_embarked_count
            with open(csv_path, 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                ...
                with open(csv_path, 'r', encoding='utf-8') as read_f:
                    ...
                    for row in reader:
                        if row and row[0] != '' and row[0] != 'RESUMEN' and 'Total' not in row[0]:
                            try:
                                cabin_num = int(row[1])
                                cabin_persons = int(row[2])
                                writer.writerow([f'cabina {cabin_num}', f'{cabin_persons} personas'])
            ...
            print(f"Total embarcados: {self.total_embarked_count}")
