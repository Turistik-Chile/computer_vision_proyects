...
if len(sys.argv) > 1:
    try:
        # Leemos el argumento como MINUTOS
        minutes_input = float(sys.argv[1])
        
        # Lo convertimos a segundos para la lógica interna
    ...
    print("👉 Ejemplo: python main.py 5   (Para 5 minutos)")
    print("👉 Ejemplo: python main.py 0.5 (Para 30 segundos)")
    sys.exit(1)
# ---------------------------------------------------------
...
try:
    with torch.no_grad(): # Ahorro de memoria
        while True:
            ...
            # 1. Chequeo de tiempo (Nota: elapsed y duration_seconds ya están ambos en segundos)
            ...
            if frame_count % 30 == 0:
                ...
                # Mostramos el progreso en segundos para que sea preciso
